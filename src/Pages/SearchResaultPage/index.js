...
function SearchResaultPage() {
	...
	function callMoviesAPI() {
		MOVIESAPI.get(
			`movies?q=${queryStrings.get("q")}&page=${queryStrings.get("page")}`
		)
			.then(function (res) {
	...
	function handlePageChange(page) {
		setLoading(true);
		setQueryStrings(
			createSearchParams({ q: queryStrings.get("q"), page: page })
		);
	}
	...
	return (
		<MainLayout>
			{loading ? (
				...
				<Fragment>
					<Title level={3}>{`Search Result of "${queryStrings.get(
						"q"
					)}"`}</Title>
...
export default SearchResaultPage;
